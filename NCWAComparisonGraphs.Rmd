...
NewComb <- read.csv("~/Desktop/GitHub/Pieris/LarvalCombData.csv", header = TRUE)
...
```{r}
ggsave("~/Desktop/GitHub/Pieris/Thesis_Figures/LarvalSurvival.png", NCWACAsurv, width = 8, height = 5)
```
...
CombTOD <- ggplot(data = TODmeans, aes(x = Treatment, y = Age.death, group = Population, linetype = Population, shape = Population, color = Population)) + 
  ...
  geom_point(size=3.5) + 
  geom_line(size = 1.5) +
  labs(x = "Mean rearing temperature (°C)", y = "Average time to death (days)", group = "Population", linetype = "Population", 
       shape = "Population")+ 
  font("xy", size = 15)+
  font("x.text", size = 12)+
  font("y.text", size = 12)+
