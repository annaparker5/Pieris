...
devrateeBOTHgraph <- ggplot(data = devrateeboth, aes(x = Trt, y = Dev.Rate.E, group = group, linetype = group, shape = group)) + 
  ...
  geom_line() + 
  labs(x = "Temperature Treatment", y = "Development Rate", group = "Experiment", linetype = "Experiment", shape = "Experiment") + 
  scale_linetype_manual(values = c("dotted", "dashed"), labels = c("Blocks 1+2", "Pupal Expt." )) + 
  scale_shape_manual(values = c(4,5), labels = c("Blocks 1+2", "Pupal Expt."))
...
surveclosBOTHgraph <- ggplot(data = survdataeclosblocks, aes(x = Treatment, y = surv.eclos.perc, group = group, linetype = group, shape = group)) + 
  ...
  geom_line() + 
  ylim(0, 1) + 
  labs(x = "Temperature treatment", y = "Proportion survival to eclosion", group = "Experiment", linetype = "Experiment", shape = "Experiment") + 
  geom_point(data = survdataeclos, size = 3) + 
  geom_line(data = survdataeclos) + 
  scale_linetype_manual(values = c("dashed", "dotted"), labels = c("Pupal Expt.", "Blocks 1+2")) + 
  scale_shape_manual(values = c(5,4), labels = c("Pupal Expt.", "Blocks 1+2"))
