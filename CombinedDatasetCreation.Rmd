...
##5. Creating a combined pupal datasheet
```{r}
...
```{r}
NCPupal <- NCPupal %>% select(Treatment, DaysHtoP, DaysPtoE, Pupate.ok, Emerge.ok)
NCPupal$Population <- "NC"
CAPupal <- CAPupal %>% select(Treatment, DaysHtoP, DaysPtoE, Pupate.ok, Emerge.ok)
CAPupal$Population <- "CA"
CAPupal26 <- CAPupal26 %>% select(Treatment, DaysHtoP, DaysPtoE, Pupate.ok, Eclose.ok)
CAPupal26$Population <- "CA"
WAPupal <- WAPupal %>% select(Trt, DaysHtoP, DaysPtoE, Pupate.ok, Emerge.ok)
