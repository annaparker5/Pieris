...
```{r}
ChamberBlocks <- lm(Time.Hatch ~ Block*Treatment, data = NCEggCham)
...
EggChamSum[nrow(EggChamSum)+1, ] <- list(34, 1, 1, 0, 0, 0, 0)
EggChamSum
...
```{r}
Eggchamdev <- ggplot(data = EggChamSum, aes(x = Avg_temp, y = Dev.rate, group = Block, linetype = Block, color = Block)) + 
  theme_bw() + 
  geom_point(size = 3) + 
  geom_line(size = 1) +
  labs(x = "Mean Temperature", y = "Development Rate (1/days)", group = "Block", linetype = "Block", 
       shape = "Block") +
  scale_linetype_manual(values = c("solid", "dashed")) + 
...
```
```{r}
ggsave("~/Desktop/GitHub/Pieris/NCChamberEggDevRate.png", Eggchamdev, width = 8, height = 5)
```
...
```{r}
eggchamsurvgraph <- ggplot(data = EggChamSurvData, aes(x = Mean.Temp, y = surv.perc, group = block, color = block, linetype = block)) + 
  theme_bw() + 
  geom_point(size = 3) + 
  geom_line(size = 1) + 
  ylim(0, 1) + 
  scale_color_manual(values = c("#1E88E5", "purple")) + 
  scale_linetype_manual(values = c("solid", "dashed")) + 
  labs(x = "Mean Rearing Temperature", y = "Proportion survival to hatching")

eggchamsurvgraph
```

```{r}
ggsave("~/Desktop/GitHub/Pieris/NCEggChamSurv.png", eggchamsurvgraph, width = 8, height = 5)
```
...
```{r}
eggpop <- lm(Dev.rate ~ Avg_temp*Population, data = CombEggGradData)
